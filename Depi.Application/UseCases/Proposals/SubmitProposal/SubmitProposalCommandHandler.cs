...
public class SubmitProposalCommandHandler : IRequestHandler<SubmitProposalCommand, ProposalResponse>
{
    ...
    {
        var freelancer = await _userRepository.GetByIdAsync(request.FreelancerId)
            ?? throw new KeyNotFoundException(Errors.NotFound("User"));

        if (_features.RequireIdentityVerification)
            freelancer.EnsureVerifiedFor("تقديم العرض");

        var project = await _projectRepository.GetByIdAsync(request.Request.ProjectId)
            ?? throw new KeyNotFoundException(Errors.NotFound("Project"));

        if (project.Status != ProjectStatus.Open)
            throw new InvalidOperationException("Project is not open for proposals");
        ...
        if (existingProposal != null)
            throw new InvalidOperationException(Errors.AlreadyExists("Proposal"));
